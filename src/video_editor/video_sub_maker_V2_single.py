...
ssl_secret_path = os.getcwd() + "/secrets/groovy_secret.json"
print(ssl_secret_path)
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = ssl_secret_path
...
def convert_mov_to_wav(video_path, audio_path):
    ...
    cmd1 = f'ffmpeg -y -i "{video_path}" -vn -acodec pcm_s16le -ac 1 "{audio_path}"'
    ...
    print(f'Mono audio extracted to {audio_path}')
...
def edit_video(abs_video_path):
    print("DOWNLOADED \nNow editing: " + abs_video_path)

    audio_path = os.getcwd() + "/audio/mono_audio.wav"
    output_srt_path = os.getcwd() + "/srt/output.srt"
    output_video_path = os.getcwd() + "/video_out/output.mov"

    mono_audio_path = convert_mov_to_wav(abs_video_path, audio_path)
    if mono_audio_path is not None:
        ...
        add_subtitles_to_video(abs_video_path, output_srt_path, output_video_path)
